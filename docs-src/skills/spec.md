...
Write a technical spec that defines a feature by what it must **never** accept,
never enter, and never produce — before a single line of implementation.
...
## Why spec before implementing

A spec written with Negative Space Programming is **directly traceable to assertions**.
Every invariant in the spec maps to one `assert.*` call in the implementation.

The spec is a contract you write once. The assertions enforce it at runtime, forever.

---

## What the skill does

The skill produces a 4-section spec after a brief interview.
...
**Entry points:** list of functions / methods / routes.
**Dependencies:** external services, repos, or state it reads.
**Hard NOs:** what this feature must NEVER do.
```

### Section 2 — Preconditions (the negative space)

Every invariant that must hold **before** the feature runs, mapped directly to assertions:
...
|:--|:--------------------------------------|:---------------------------------------|
| Q1 | receipt must have a transactionId    | assert.notNil(receipt.transactionId)   |
| Q2 | receipt.amount must equal amount     | assert.equal(receipt.amount, amount)   |
```

### Section 4 — State machine (if applicable)

For features with entity lifecycle transitions, the spec includes a state transition table with valid/invalid transitions and the assertion that guards each one.
...
2. **Preconditions table** — invariant → violated by → assert with
3. **Postconditions table** — output guarantees → assert with
4. **Implementation checklist** — one checkbox per assertion, tracing spec to code
